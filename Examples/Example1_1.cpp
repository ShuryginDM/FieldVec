/**
* Вывод всех нормированных многочленов над кольцом вычетов из mod степени не выше n и вывод их числа.
*/
/*
* Пример использования библиотеки FieldVec.
* Строит все возможные неприводимые нормированные многочлены над кольцом вычетов из mod степени не выше n,
* а также считает их число
...
int main(){
    ...
    if(mod == 2){/* 
                    BoolVec лучше оптимизирован. Поэтому при mod == 2 лучше использовать его
                    Помимо этого, при mod == 2 любой полином, отличный от 0, является нормированным полиномом степени n
                 */
    ...
    while(r.size() - 1 <= n){ /* В r.size() учитывается также и свободный коэффициент */
	if(r[r.size() - 1] != 1){ /* 
                                     Нас интересуют только нормированные многочлены. Значит коэффициент
                                     при наивысшей степени должен быть равен 1. Если оказалось, что он не
                                     равен 1, то он впервые попадет в эту ветку равным 2, а все остальные
                                     коэффициенты будут равны 0. Поэтому простым обнулением коэффициента при
                                     текущей максимальной степени и установление значения 1 для следующей за
                                     текущей максимальной степени приведет нас к числу вида q ^ n
                                  */
	    r[r.size() - 1] = 0;
            r.push_back(1);
            continue;
        ...
        b = false;
        for(int i = 0; i < v.size(); i++){
            if((v[i].size()) * 2 > r.size() + 2){ /* достаточно просмотреть многочлены, степень квадрата которых не превышает данную*/
                break;
            }
            if(nl == (r % v[i])){
                b = true;
